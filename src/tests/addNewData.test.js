...
describe('countNewTotal', () => {
  test('if price index in the bids will be 0, then total = amount', () => {
    ...
    const newAmount = 0.9870

    expect(countNewTotal(orderList, bookMap, desiredIndex, newAmount)).toBe(0.9870);
  });
  test('if price index in the bids will be 2, it count the right way', () => {
    ...
    expect(countNewTotal(orderList, bookMap, desiredIndex, newAmount)).toBe(expected);
  });
})
